...
class Run3nuMC : public edm::EDAnalyzer {
public:
  /// Constructor
  explicit Run3nuMC(const edm::ParameterSet&);
  /// Destructor
  virtual ~Run3nuMC();
...
private:
  ...
  virtual float ExtractPileUpWeight(Int_t pu);
  virtual void endJob();
...
Run3nuMC::Run3nuMC(const edm::ParameterSet& iConfig) :
...
void Run3nuMC::beginRun(edm::Run const& iRun, edm::EventSetup const& iSetup)
{
...
void Run3nuMC::beginJob()
{
  ...
  this -> _tree -> Branch("TruePU",  &_nTruePU);
  this -> _tree -> Branch("Run3RateWeight", &_Run3RateWeight);
...
void Run3nuMC::endJob()
{
  return;
}


void Run3nuMC::endRun(edm::Run const& iRun, edm::EventSetup const& iSetup)
{
  return;
}


void Run3nuMC::analyze(const edm::Event& iEvent, const edm::EventSetup& eSetup)
{
  this -> Initialize();
  ...
  if(PUinfoHandle.isValid()) {
    _nTruePU = PUinfoHandle->begin()->getTrueNumInteractions();
    _Run3RateWeight = ExtractPileUpWeight(_nTruePU);
  }
...
float Run3nuMC::ExtractPileUpWeight(Int_t pu)
{
  // The following has been taken from https://github.com/cms-l1-dpg/L1MenuTools/blob/30ea05868d36f1f21ffa8e2ceda03d3cfbf52dee/rate-estimation/plots/CompPUDep.py#L92-L102
  // The weights are based on /SingleNeutrino_Pt-2To20-gun(E10-gun)/Run3Winter21DRMiniAOD-FlatPU30to80FEVT_SNB_112X_mcRun3_2021_realistic_v16-v2/GEN-SIM-DIGI-RAW dataset
  //
...
Long64_t Run3nuMC::FindTriggerBit(const vector<string> foundPaths, const vector<int> indexOfPaths, const edm::Handle<edm::TriggerResults>& triggerResults){
